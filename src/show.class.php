...
class Show extends GlobalClass
{
	...
	function EnrollData()
	{
		...
		$this->datum_m = mysqli_date($this->_datum);

		//Enroll
		$query18 = mysqli_query($rDbConn, "SELECT *, DATE_FORMAT(exp_date,'%d-%m-%Y') as date FROM enroll WHERE exp_date='".$this->_datum_m."' ");
		$aantal = mysqli_num_rows($query18);
		if ($aantal < 1)
		{
			echo ('
		...
		{
			$obj18 = mysqli_fetch_array($query18);
			echo ('
		...
		foreach($user->getAllUsers() as $user2){
			...
			$query20 = mysqli_query($rDbConn, $querystring20);
			$aantal = mysqli_num_rows($query20);
			if ($aantal < 1)
			{
				echo ('
			...
			{
				while ($obj20 = mysqli_fetch_array($query20))
				{
					echo ('
		...
		//sum nb uit e_detail
		$query21 = mysqli_query($rDbConn, "SELECT SUM(nb) AS nb FROM e_detail WHERE exp_date='".$this->_datum_m."'");
		$obj21 = mysqli_fetch_array($query21);
		echo ('
	...
	function OverviewCost()
	{
		...
		foreach($user->getAllUsers() as $user2){
			$query2 = mysqli_query($rDbConn, "SELECT ROUND(IFNULL(SUM(exp),0),2) AS paid FROM expenses WHERE user_id='".$user2->userid."' AND exp_date >= '$this->_exp_from' ");
		...
		$query3 = mysqli_query($rDbConn, "SELECT ROUND(IFNULL(SUM(exp),0),2) AS paid FROM expenses WHERE exp_date >= '$this->_exp_from' ");
		...
		foreach($user->getAllUsers() as $user2){
			$query5= mysqli_query($rDbConn, "SELECT ROUND(IFNULL(SUM(exp_pu),0),2) AS debt FROM exp_detail WHERE user_id='".$user2->userid."' AND exp_date >= '$this->_exp_from'");
			$obj5= mysqli_fetch_array($query5);
			echo('
				<td class="h3" align="center">'.$obj5[debt].'</td>');
		}

		//Berekenen schuld totaal
		$query6= mysqli_query($rDbConn, "SELECT ROUND(IFNULL(SUM(exp_pu),0),2) AS debt FROM exp_detail WHERE exp_date >= '$this->_exp_from'");
		$obj6=mysqli_fetch_array($query6);
		echo ('
		...
		//Saldo
		foreach($user->getAllUsers() as $user2){
			$query8 = mysqli_query($rDbConn, "SELECT ROUND(IFNULL(SUM(exp),0),2) AS paid FROM expenses WHERE user_id='".$user2->userid."' AND exp_date >= '$this->_exp_from' ");
			$query9= mysqli_query($rDbConn, "SELECT ROUND(IFNULL(SUM(exp_pu),0),2) AS debt FROM exp_detail WHERE user_id='".$user2->userid."' AND exp_date >= '$this->_exp_from'");
			$obj8 = mysqli_fetch_array($query8);
			$obj9 = mysqli_fetch_array($query9);
	...
	function ShowData()
	{
		...
		// --> Inhoud tabel
		$query11 = mysqli_query($rDbConn, "SELECT *, DATE_FORMAT(exp_date,'%d-%m-%Y') as date FROM expenses ORDER BY exp_date DESC ".$this->_show_records." ");
		while ($obj11 = mysqli_fetch_array($query11))
		{
			echo ('
			 <tr>
			...
			foreach($user->getAllUsers() as $user2){
				$query12 = mysqli_query($rDbConn, "SELECT nb FROM exp_detail WHERE exp_id='".$obj11['id']."' AND user_id='".$user2->userid."'");
				$aantal = mysqli_num_rows($query12);
				if ($aantal < 1)
				{
					echo ('
					<td class="h3" width ="63" align="center">0</td>');
				...
				{
					while ($obj12 = mysqli_fetch_array($query12))
					{
						echo ('
		...
		//Rij met totalen
		foreach($user->getAllUsers() as $user2){
			$query14 = mysqli_query($rDbConn, "SELECT COUNT(nb) AS totaal FROM exp_detail WHERE user_id='".$user2->userid."' AND nb > 0 AND exp_date >= '$this->_exp_from'");
