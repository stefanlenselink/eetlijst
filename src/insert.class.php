...
class Insert extends GlobalClass
{
	...
	function insert_exp()
	{
		...
		$query = mysqli_query($rDbConn, "Select id FROM ".$this->_table_1." ORDER BY id DESC LIMIT 1");
		...
		foreach ($this->_nb as $z => $q)
		{
			...
			$Result = mysqli_query($rDbConn, $query) or die(mysqli_error($rDbConn));
		}
	...
	function update_exp()
	{
		...
		$query= mysqli_query($rDbConn, "DELETE FROM ".$this->_table_2." WHERE exp_id='".$this->_edit_id."' ");
	...
	function insert_enroll()
	{
		...
		foreach ($this->_nb as $z => $q)
		{
			...
			$query2 = mysqli_query($rDbConn, $querystring2) or die(mysqli_error($rDbConn));
		}
	}
	...
	{
	    $rDbConn = connectdb();
		$query	 = mysqli_query($rDbConn, "DELETE FROM ".$this->_table_1." WHERE id=".$this->_edit_id." ");
		$query	 = mysqli_query($rDbConn, "DELETE FROM ".$this->_table_2." WHERE exp_id=".$this->_edit_id." ");
	}
