...
if(FALSE!==($rDbConn=connectdb()))
{
	...
	$query2 = mysqli_query($rDbConn, $querystring2);
		while ($obj2 = mysqli_fetch_array($rDbConn, $query2)) {
		...
		$obj4 = mysqli_fetch_array($query4);

		//insert
		$saldo = $obj3['paid'] - $obj4['debt'];
		$querystring5 = "INSERT INTO tmp_list_1 (user_id, saldo) VALUES ('$id', '$saldo')";
		$query5 = mysqli_query($rDbConn, $querystring5);

	}

	$query = mysqli_query($rDbConn, $querystring2);
	...
	for ($i = 1; $i <= $z; $i++) {
		...
		if (($obj1['max'] != '0') && ($obj1['min'] != '0')) {
			...
			$query5 = mysqli_query($rDbConn, $querystring5);
			$query6 = mysqli_query($rDbConn, $querystring6);
			}
	...
	$query2= mysqli_query($rDbConn, $querystring2);
	while ($obj2 = mysqli_fetch_array($query2)){
